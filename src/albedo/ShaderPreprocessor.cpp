...
namespace albedo {
    ShaderPreprocessor::ShaderPreprocessor(fs::path base_path, const fs::path &name)
    ...
    std::string ShaderPreprocessor::str() const {
        return result;
    }
    const GLchar *const *ShaderPreprocessor::c_str() const {
        return &c_result;
    }
